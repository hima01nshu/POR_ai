...
def hash_all_blocks(output_folder, output_folder_hash, file_name):
    """Hash all blocks in the output folder and store the hashes for each file."""
    file_hash_folder = os.path.join(output_folder_hash, os.path.splitext(file_name)[0])
    if not os.path.exists(file_hash_folder):
    ...
    hash_file_path = os.path.join(file_hash_folder, 'block_hashes.txt')
    with open(hash_file_path, 'w') as hash_file:
        for block_file, block_hash in block_hashes.items():
            hash_file.write(f"{block_file}: {block_hash}\n")
    
    logging.info(f"Hashes stored for {file_name} in {hash_file_path}")
...
def process_all_files(input_folder, block_size, output_folder, output_folder_hash):
    ...
    for file_name in os.listdir(input_folder):
        ...
        if os.path.isfile(file_path):
            ...
            hash_all_blocks(file_output_folder, output_folder_hash, file_name)
